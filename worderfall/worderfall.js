...
var instruments = [
    " ", "perc_drum_snare", "perc_drum_bass", "perc_drum_tom1",
    "perc_cymbal_crash3", "perc_hihat_closed", "perc_bongo_low",
    "perc_cowbell_low", "perc_cowbell_high", "xylo_c5", "xylo_d5", "xylo_e5", "xylo_f5",
    "xylo_g5", "xylo_a5", "xylo_b5", "xylo_c6", "l_piano_c4", "l_piano_d4", "l_piano_e4",
    "l_piano_f4", "l_piano_g4", "l_piano_a4",
    "l_piano_b4", "l_piano_c5"
];

var songA = [0,2,0,2,0,2,0,1];
var songB = [0,9,0,10,0,11,0,14];
var songC = [23, 21, 19, 24, 22, 20, 21, 19];
var songTimer = 0;
var combo = 0;
var numAnswers = 4;
...
var isWordValid = function(word){
    for(var i = 0; i<words.length; i++){
        var count = 0;
        var canBeWrong = false;
        for(var j = 0; j<5; j++){
            if(words[i].substring(j, j+1)==word.substring(j,j+1)){
                count+=1;
            }
            if(count==3){
            	canBeWrong = true;
...
var genProblem = function(time, difference){

	var word = words[PS.random(words.length-1)];
	while(!isWordValid(word)){
        word = words[PS.random(words.length-1)];
	}
	var words1 = new Array();
	var words2 = new Array();
	var words3 = new Array();
	var words4 = new Array();
	var words5 = new Array();
	...
	for(var i = 0; i<numAnswers; i++){
		var randomnum = PS.random(words3.length)-1;
		answers.push(words3[randomnum]);
	}
	var answerIndex = PS.random(answers.length)-1;
	answers[answerIndex] = words4[PS.random(words4.length)-1];
	for(var i = 0; i<answers.length; i++) {
		if (i == answerIndex){
			printWord(answers[i], (((6 * i) % 12) + 2), 4 + i / 2, "true");
		}
		else {
			printWord(answers[i], (((6 * i) % 12) + 2), 4 + i / 2, "false");
        }
    }
}
...
var count = function(){
	...
	counter++;
	printNumber(timer+"",3,0,0);
	printNumber(score+"", 12,0,0);
	if(timer>0 && counter%12==0) {
        timer--;
    }

    if(counter%5==0){
    	songTimer++;
    	if(score>0 && (songA[songTimer % songA.length])!=0) {
            PS.audioPlay(instruments[(songA[songTimer % songA.length])]);
        }
        if(score>50 && (songB[songTimer % songB.length])!=0){
            PS.audioPlay(instruments[(songB[songTimer % songB.length])], { volume: 0.3 });
		}
        if(score>100 && (songC[songTimer % songC.length])!=0){
            PS.audioPlay(instruments[(songC[songTimer % songC.length])], { volume: 0.25});
        }
    }
	...
	if(timer<=0){
		...
		printWord("Game Over", 3,3, 0);
		printWord("Score: "+score,3,5,0);
		printWord("New game?", 3, 6, "new");
        PS.color(PS.ALL, PS.ALL, PS.COLOR_WHITE);
        PS.glyphColor(PS.ALL, PS.ALL, PS.COLOR_BLACK);
        PS.timerStop(timerHolder);
	}
}

var printWord = function(word, x, y, data){
	try {
        for (var i = 0; i < word.length; i++) {
            PS.glyph(x + i, y, word.charAt(i));
            PS.data(x + i, y, data);
        }
    }
    catch (e) {
...
var printNumber = function(word, x, y, data){
    PS.glyph(x-2, y, 0);
    try {
        for (var i = word.length-1; i >=0; i--) {
            PS.glyph(x - word.length+i, y, word.charAt(i));
            PS.data(x - word.length+i, y, data);
        }
    }
    catch (e) {
        genProblem(1,1);
...
var newGame = function(){
    ...
    PS.data(PS.ALL, PS.ALL, 0);
    PS.glyph(PS.ALL, PS.ALL, "");
    printWord("Ready to play?", 1,3, 0);
    printWord("Start", 5, 6, "new");
}
...
PS.init = function( system, options ) {
	...
	PS.statusText( "Select a word that changes only 1 letter" );
	newGame();
...
PS.touch = function( x, y, data, options ) {
	...
	if(data=="true"){
        ...
        counter = 0;
        score=0;
        timerHolder = PS.timerStart(5, count);
        genProblem(1,1);
        combo = 0;
	}
...
PS.enter = function( x, y, data, options ) {
	...
	if(data == "false"||data=="true"){
        for(var i = 0; i<5; i++){
        	PS.color(start+i, y, PS.COLOR_BLACK);
        	PS.glyphColor(start+i, y, PS.COLOR_WHITE);
		}
	}
};
...
PS.exit = function( x, y, data, options ) {
    ...
    if(data == "false"||data=="true"){
        for(var i = 0; i<5; i++){
            PS.color(start+i, y, PS.COLOR_WHITE);
            PS.glyphColor(start+i, y, PS.COLOR_BLACK);
        }
    }
};
